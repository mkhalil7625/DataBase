...
c.execute("SELECT {idf} FROM {tn} WHERE {cn} BETWEEN '2013-03-06 10:10:10' AND '2019-03-06 10:10:10'".\
            format(idf=id_field, tn=table_name, cn=date_time_col))
all_date_times = c.fetchall()
print('4) all entries between ~2013 - 2019:', all_date_times)

# 5) Retrieve all IDs of entries between that are older than 0 day and 0 hrs
c.execute("SELECT {idf} FROM {tn} WHERE DATE('now') - {dc} >= 0 AND DATE('now') - {tc} >= 0".\
